...
void verify_program(_In_ const char* file, uint32_t expected_section_count)
{
    uint32_t result;
    ...
    while (true) {
        ...
        REQUIRE(
            (result = ebpf_api_elf_verify_section_from_file(
                file, section_name, nullptr, false, &report, &error_message, &stats),
...
TEST_CASE("verify_snat_program", "[cilium][xdp]") { verify_program(SAMPLE_PATH "bpf_xdp_snat.o", CILIUM_XDP_SECTIONS_SNAT); }
TEST_CASE("verify_dsr_program", "[cilium][xdp]") { verify_program(SAMPLE_PATH "bpf_xdp_dsr.o", CILIUM_XDP_SECTIONS_DSR); }
